...
if [ "$1" == "--auto-teacher-att" ]; then
    ...
    SYSTEM_LOG="$BASE_DIR/system_log.csv"
    ...
    if [ ! -f "$SYSTEM_LOG" ]; then
        echo "DATE,TIME,ROLE,USER,ACTION,STATUS,DETAILS" > "$SYSTEM_LOG"
    fi

    echo "$date_today,$time_now,SYSTEM,-,CRON,START,Teacher attendance started" >> "$SYSTEM_LOG"

    while IFS=: read -r tid tname temail tpass; do

        if grep -q "^$date_today:$tid:" "$TEACHER_ATT_FILE"; then
            echo "$date_today,$time_now,TEACHER,$tid,$tname,ATTENDANCE,SKIP,Already marked" >> "$SYSTEM_LOG"
        else
            echo "$date_today:$tid:Absent" >> "$TEACHER_ATT_FILE"
            echo "$date_today,$time_now,TEACHER,$tid,$tname,ATTENDANCE,ABSENT,Auto marked" >> "$SYSTEM_LOG"
        fi

    done < "$TEACHER_FILE"

    csv_file="$BASE_DIR/teacher_attendance_$date_today.csv"

    echo "Date:TeacherID:Status" > "$csv_file"
    cat "$TEACHER_ATT_FILE" >> "$csv_file"

    echo "$date_today,$time_now,SYSTEM,-,CSV,GENERATED,$csv_file" >> "$SYSTEM_LOG"

    echo "$date_today,$time_now,SYSTEM,-,CRON,END,success" >> "$SYSTEM_LOG"

    echo "✅ All activity stored in system_log.csv"
    exit 0
fi
...
if [ "$role" == "Teacher" ]; then
    ...
    # If record exists → update to Present
    if grep -q "^$date_today:$username:" "$TEACHER_ATT_FILE"; then
        sed -i "s/^$date_today:$username:.*/$date_today:$username:Present/" "$TEACHER_ATT_FILE"
    else
        echo "$date_today:$username:Present" >> "$TEACHER_ATT_FILE"
    fi
fi
...
# ================= CHANGE PASSWORD (FIXED ROLE-BASED) =================
change_password() {
    case "$role" in
    ...
    echo -e "${YELLOW}📩 Sending OTP to your email...${RESET}"
    email=$(echo "$user_data" | cut -d ":" -f3)
    ...
    case "$role" in
        Admin)
            awk -F: -v OFS=: -v user="$username" -v pass="$newpass" '
            $1==user { $2=pass }
            { print }
...
forget_password() {
    ...
    echo -e "${YELLOW}📩 Sending OTP to $email...${RESET}"
    otp=$(send_otp "$email")
    ...
    # ================= ROLE BASED SAFE UPDATE =================
    case $role_type in
        admin)
            # ADMIN FORMAT: id:password:email  OR id:name:email:password ?
            # safe detection by field count
            fields=$(echo "$user_record" | awk -F: '{print NF}')

            if [ "$fields" -eq 3 ]; then
                # id:password:email
                awk -F: -v OFS=: -v user="$uname" -v pass="$newpass" '
                $1==user { $2=pass }
                { print }
                ' "$file" > temp.txt && mv temp.txt "$file"
            else
    ...
    user_data=$(grep "^$uname:" "$file")

    echo -e "${GREEN}✅ Password reset successful!${RESET}"
}
...
rm -f temp_search.txt
            ...
            13)
                ...
                case $a_choice in
                    1)
                        ...
                        ;;
                    2)                      
                        #nl -w2 -s'. ' "$ANNOUNCEMENT_FILE"
			#read -p "Enter line number to delete: " line
			#sed -i "${line}d" "$ANNOUNCEMENT_FILE"
			#echo -e "${GREEN}✅ Announcement deleted${RESET}"
                        #;;
			...
			total=$(wc -l < "$ANNOUNCEMENT_FILE")

			if ! [[ "$line" =~ ^[0-9]+$ ]]; then
			    echo -e "${RED}❌ Invalid input (not a number)${RESET}"

			elif [ "$line" -lt 1 ] || [ "$line" -gt "$total" ]; then
			    ...
			    echo -e "${GREEN}✅ Announcement deleted${RESET}"
                        ...
                        #if grep -q "^$del_date:$teacher_name:" "$ANNOUNCEMENT_FILE"; then
                        #sed -i "/^$del_date:$teacher_name:/d" "$ANNOUNCEMENT_FILE"
                        #echo -e "${GREEN}✅ Delete successful${RESET}"
			#else
    			#echo -e "${RED}❌ Insufficient data / Announcement not found${RESET}"
			#fi
			#;;
...
while true; do
   ...
   # sleep 3
    echo -ne "${GREEN}Loading menu${RESET}"
    sleep 3
    for i in {1..3}; do
      echo -ne "."
      sleep 0.5
